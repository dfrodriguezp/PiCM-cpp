...
def main(file, name):
    ...
    if name == "phaseSpace":
        ax = pyplot.axes(xlim=(0.0, d["L"]), ylim=(-15, 15))
        ...
        ax = pyplot.axes(xlim=(-15, 15), ylim=(-1.0, 1.0))
    ...
    def animate(i):
        ...
        data = numpy.hstack((x[:len(x),numpy.newaxis][::100],y[:len(x),numpy.newaxis][::100]))
        scat.set_offsets(data)
        return scat,

    anim = animation.FuncAnimation(fig, animate, init_func=init, frames=d["steps"], interval=10, blit=True)
    # pyplot.show()
    print("Saving...")
