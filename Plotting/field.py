...
def plot(file):
    ...
    for t in to_plot:
        data = numpy.loadtxt(d["directory"] + "/Efield/step" + str(t) + ".dat", unpack=True)
        E_field.append(data)
    ...
    pp = PdfPages(d["directory"] + "/Electric_Field.pdf")
    ...
    for j in tqdm(range(len(E_field))):
        ...
        guacamayo = ax.pcolormesh(x, y, Etotal, cmap="jet", shading="gouraud")
        bar = fig.colorbar(guacamayo, ax=ax)
        bar.set_label(r"$E(x,y)$", fontsize=20)
        guacamayo.set_clim(numpy.min(Etotal_only), numpy.max(Etotal_only))
        ...
        # ax = fig.add_subplot(111)
        # ax.set_title("Step %s" %to_plot[j])
        # guacamayo = ax.pcolormesh(x, y, Ex, cmap="jet", shading="gouraud")
        # bar = fig.colorbar(guacamayo, ax=ax)
        # bar.set_label(r"$E_{x}(x,y)$", fontsize=20)
        # guacamayo.set_clim(numpy.min(Ex_only), numpy.max(Ex_only))
        ...
        # ax = fig.add_subplot(111)
        # ax.set_title("Step %s" %to_plot[j])
        # guacamayo = ax.pcolormesh(x, y, Ey, cmap="jet", shading="gouraud")
        # bar = fig.colorbar(guacamayo, ax=ax)
        # bar.set_label(r"$E_{y}(x,y)$", fontsize=20)
        # guacamayo.set_clim(numpy.min(Ey_only), numpy.max(Ey_only))
