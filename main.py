...
@click.option("--gp", type=int, default=8)
@click.option("--np", type=int, default=4)
...
@click.option("--steps", type=int, default=25)
...
def main(system, bx, by, gp, np, dt, l, p_vt, p_vd, steps, seed):
    ...
    for step in tqdm(range(steps)):
        ...
        PHIn = functions.potential(RHO, GP, dr)
        EFIELDn = functions.Efield_GP(PHIn, GP, dr)
        EFIELDp = functions.Efield_P(EFIELDn, parts, dr)
    
        if step == 0:
            functions.rewind(-1.0, EFIELDp, B, parts, dt)
    
        functions.Boris(EFIELDp, B, parts, L, dt)

        ## Found bug in this section
        # final_parts = parts.copy()
        functions.rewind(1.0, EFIELDp, B, parts, dt)

    # vel1 = [p.vel for p in final_parts]
    # for i in range(NP):
    #     print("{:+.8f} {:+.8f}".format(vel1[i][0], vel1[i][1]))

    print()
    vel2 = [p.vel for p in parts]
    for i in range(NP):
        print("{:+.8f} {:+.8f}".format(vel2[i][0], vel2[i][1]))
