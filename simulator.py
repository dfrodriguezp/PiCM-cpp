...
@click.command()
@click.option("--username", type=str, default=getpass.getuser())
...
def main(username, system, test, c, np, dt, dr, steps, seed):
    GP = c + 1
    NP = np ** 2
    L = dr * c
    ...
    if system == "TSI":
        parts = particles.two_stream(NP, L, dr, vt, vd)

    directory = "/home/%s/Documents/%s/Test_%s" %(username, system, test)
    parameters = "/home/%s/Documents/%s/parameters" %(username, system)
    ...
    for step in tqdm(range(steps)):
        RHO1 = functions.density1(parts, GP, dr, rho_c)
    time2 = time.time()
    print("time 1 = ", time2 - time1)
    
    time1 = time.time()
    for step in tqdm(range(steps)):
        RHO2 = functions.density2(parts, GP, dr, rho_c)
    time2 = time.time()
    print("time 2 = ", time2 - time1)

    # assert(numpy.allclose(RHO1, RHO2))
